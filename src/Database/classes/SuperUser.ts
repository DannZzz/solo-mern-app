...
export type SuperUserJson = Omit<Database.User, DangerFields> & {
...
export default class SuperUser {
  ...
  async messagesWith(id: string) {
    // const _messages = await this.fetchField("messages");
    // return [...(_messages?.[id] || [])];
  }
  ...
  async fetchField<K extends keyof Database.User>(
    key: K
  ): Promise<Database.User[K]> {
  ...
  getAccessType(userId: string, accessType: PostAccessType): PostAccessType {
    ...
    switch (accessType) {
      ...
      case "followers":
        return Boolean(
          (this?._data?.followers || []).find((data) => data._id === userId)
        )
          ? accessType
          : "all";
      ...
      case "following":
        return Boolean(
          (this?._data?.following || []).find((data) => data._id === userId)
        )
          ? accessType
          : "all";
  ...
  field<K extends keyof SuperUserJson>(
    field: K & string
  ): SuperUserJson[K] | any {
  ...
  async fetchEmail(cache?: true) {
    ...
    const email = (await Database.getModel("User").findOne("_id", this._id))
      .email;
