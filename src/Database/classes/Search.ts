...
export const getUsers = async (exlcudeIds: string[] = []) => {
  const res = await Database.getModel("User").find(
    (data) => new SuperUser(data)
  );
  return Array.isArray(res)
    ? res?.filter((user) => !exlcudeIds.includes(user._id))
    : [];
};
...
export async function searchQuery(
  query: string,
  exlcudeIds: string[] = []
): Promise<{ users: SuperUserJson[]; characteristicUsers: SuperUserJson[] }> {
  const result = {
    byUsername: [],
    byName: [],
    characteristicUsers: [],
  ...
  users.forEach((user) => {
    switch (query[0]) {
      case symbolGroup.characteristicUsers:
        if (
          user?.characteristics?.some((charObj) =>
            charObj.text?.toLowerCase().includes(query.slice(1))
          )
        )
          result.characteristicUsers.push(user.data());

      default:
        if (user?.username?.toLowerCase().includes(query))
          result.byUsername.push(user.data());
        else if (user?.name?.toLowerCase()?.includes(query))
          result.byName.push(user.data());
    }
  });
