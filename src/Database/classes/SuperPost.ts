...
export type SuperPostJson = Database.Post & { likeCount?: number, commentCount?: number };

export type PostAccessType = Database.Post['accessType'];

const fields: Array<keyof Database.Post> = ['_id', "accessType", "attachments", "comments", "createdAt", "creatorId", "edits", "likes", "text"];
...
export default class SuperPost {
    ...
    constructor(data: Database.Post) {
        ...
        _data.attachments = (_data.attachments || []).map(file => Config.get('PUBLI_POST_ATTACHMENT_API') + file)
    ...
    data(accessType: PostAccessType): Partial<SuperPostJson> {
        ...
        const notDangerFields: Array<keyof SuperPostJson> = ['creatorId', 'createdAt', 'likeCount', 'commentCount', "accessType"];
        const data = {};
