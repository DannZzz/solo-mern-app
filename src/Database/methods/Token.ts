...
export const token_add = async (id: string) => {
  ...
  while (_tokens.logins.find((obj) => obj._token === token))
    token = uuid(50, { aditional: "_$@#%^&*!" });
  await Database.getModel("User").updateOne(id, {
    field: "logins",
    value: { date: new Date(), _token: token },
    method: "$push",
  });
  return token;
};
...
export const token_check = async (token: string, id: string) => {
  const _tokens = (await userData(id, true))?.logins || [];

  return Boolean(
    _tokens.find((obj) => obj._token === token && checkDate(obj.date))
  );
};
...
export const token_checkWithUsername = async (
  token: string,
  username: string
) => {
