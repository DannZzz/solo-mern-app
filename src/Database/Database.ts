...
export namespace Database {
  ...
  export const UserMessage = model(
    "usermessages",
    new Schema<UserMessage>({
      _id: String,
      open: { type: Array as any, default: [] },
      members: { type: Array as any, default: [] },
      messages: { type: Array as any, default: [] },
      changedAt: Date,
    })
  );
  ...
  export const models = {
    User: Database.User,
    Post: Database.Post,
    UserMessage: Database.UserMessage,
  };
  ...
  class Model<T extends keyof Database.models, I extends Database.models[T]> {
    ...
    findOne(filter: FilterQuery<I>): Promise<I>;
    findOne<K extends keyof I>(key: K, value: I[K]): Promise<I>;
    async findOne<K extends keyof I>(
      key: K | FilterQuery<I>,
      value?: I[K]
    ): Promise<I> {
      try {
        ...
        if (typeof key === "string") {
          res = await this.data.findOne({ [key]: value });
        } else {
          res = await this.data.findOne(key);
        }
        if (res) {
          return res as any;
        } else return null;
    ...
    find<T extends any>(
      ...
      converter: (data: I | any) => T
    ): Promise<T[]>;
    async find<T extends any>(
      convOrFilter?: FilterQuery<I> | ((data: I | any) => T),
      converter?: (data: I | any) => T
    ): Promise<T[] | I[]> {
      try {
        const res = await this.data.find(
          typeof convOrFilter !== "function" ? convOrFilter : {}
        );
        if (res) {
    ...
    updateOne<E extends keyof I, M extends "$set" | "$inc" | "$push">(
      filter: string,
      update: {
        field: E;
        value: M extends "$push"
          ? I[E] extends any[]
            ? I[E][number]
            : I[E]
          : I[E];
    ...
    >(
      ...
      update: { field: E; value: I[E]; method?: M }
    ): Promise<boolean>;
    async updateOne<K extends keyof I, E extends keyof I>(
      filter: string | { field: K; value: I[K] },
      update: { field: E; value: I[E]; method?: "$set" | "$inc" | "$push" }
    ): Promise<boolean> {
      try {
        await this.data.updateOne(
          {
            [typeof filter === "string" ? "_id" : filter.field]:
              typeof filter === "string" ? filter : filter.value,
          },
          { [update.method || "$set"]: { [update.field]: update.value } }
        );
        return true;
      } catch {
        return false;
