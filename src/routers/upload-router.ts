...
const avatarsMulterConfig = multer.diskStorage({
  ...
  filename(req, file, cb) {
    const name = `${req.body._id}-${uuid(50)}${file.originalname.slice(
...
const postImagesMulterConfig = multer.diskStorage({
  ...
  filename(req, file, cb) {
    const name = `${Date.now()}-${uuid(50)}${file.originalname.slice(
      file.originalname.lastIndexOf(".")
    )}`;
    cb(null, name);
  },
...
const postImagesMulter = multer({
  storage: postImagesMulterConfig,
  fileFilter(req, file, callback) {
...
UploadRouter.post(
  ...
  async (req, res) => {
    ...
    const file = req.file;
    if (!file || !_token || !_id)
      return fieldsRequired(res, "avatar", "_token", "_id");
...
UploadRouter.post(
  "/post/new",
  postImagesMulter.array("attachments"),
  async (req, res) => {
    const json = Json(res);
    ...
    if (!_token || !_id || !text)
      return fieldsRequired(res, "text", "_token", "_id");
    ...
    const isCreated = await createPost({
      accessType: ["all", "followers", "following"].includes(privacyType)
        ? privacyType
        : "all",
      text,
      attachments: files,
