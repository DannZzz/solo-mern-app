...
export const fetchUser = createAsyncThunk<
  CurrentUser & { fn?: CallableFunction },
  { _token: string; _id: string; fn?: CallableFunction }
>("currentUser/fetchUser", async ({ _id, _token, fn }) => {
...
export const currentUserSlice = createSlice({
  ...
  reducers: {
    ...
    changeCurrentUserDetail: <K extends keyof UserDetails>(
      state: CurrentUser,
      action: PayloadAction<{ key: K; value: UserDetails[K] }>
    ) => {
...
export const { takeCurrentUser, changeCurrentUserDetail, setUser, removeUser } =
...
export function isCurrentUser(checkDetails: boolean = false) {
  return Boolean(
    getCurrentUserLocalStorage()?.[checkDetails ? "details" : "_token"]
  );
}
