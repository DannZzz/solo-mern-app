...
import { setUser, fetchUser } from '../../features/current-user/current-user-slice';
...
const Register = () => {
    ...
    async function register(data: { username: string, password: string, email: string }) {
        ...
        dispatch(addAlert({ endsIn: 4000, message: res.message, type: res.status === "OK" ? 'success' : 'danger' }))
        if (res.status === "OK") {
            dispatch(setUser({ _token: res._token, _id: res._id }));
            dispatch(fetchUser({ _token: res._token, _id: res._id, fn: () => window.location.replace("/") }) as any)
        }
    }
    ...
    async function handleUsernameTyping(username: string) {
        ...
        function doThis(element: any) {
            ...
            if (usernameValid.find(err => element.classList.contains("register-" + err))) {
    ...
    function handlePasswordTyping(password: string) {
        ...
        forEachChild(passwordValidityRef.current, element => {
            if (passwordValid.find(err => element.classList.contains("register-" + err))) {
                element.classList.remove("register-valid");
            } else {
                element.classList.add("register-valid");
            }
    ...
    function handleEmailTyping(email: string) {
        ...
        forEachChild(emailValidityRef.current, element => {
            if (emailValid.find(err => element.classList.contains("register-" + err))) {
                element.classList.remove("register-valid");
            } else {
                element.classList.add("register-valid");
            }
    ...
    return (
        <div className="register">
            <h1>Sign Up - <span>Origin</span></h1>
            <form className='register-form'>
                <div className="register-input">
                    ...
                    <input type="text" id="register-username" placeholder='username' onKeyUp={e => handleUsernameTyping(usernameRef.current)} onChange={e => usernameRef.current = e.target.value} />
                </div>
                <div className="register-input">
                    <div className="register-validity-block" ref={emailValidityRef}>
                        <span className="register-invalid">Looks like an email</span>
                    </div>
                    <input type="email" id="register-email" placeholder='email' onKeyUp={e => handleEmailTyping(emailRef.current)} onChange={e => emailRef.current = e.target.value} />
                </div>
                <div className="register-input">
                    ...
                    <input type="password" id="register-password" placeholder='password' onKeyUp={e => handlePasswordTyping(passwordRef.current)} onChange={e => passwordRef.current = e.target.value} />
                </div>
                <button className="register-submit-button" onClick={handleSubmit}>Sign Up</button>
                <span className="register-footer">Already registered? <a href="/login" className='prevent-default-a'>Login</a></span>
            </form>
        </div>
    )
}
